...
@HiltViewModel
class CategoryViewModel @Inject constructor(private val repository: CategoryRepository, private val state: SavedStateHandle) : ViewModel() {

    private val currentData = state.getLiveData(CURRENT_QUERY, DEFAULT_QUERY)

    val categoryPhotos = currentData.switchMap { id ->
        repository.getCategoryPhotos(id).cachedIn(viewModelScope)
    }
